...
by $\Delta$AUC\,=\,+XX across five LOBO folds.
...
Our mean spatial-CV AUC of XX surpasses the single-basin benchmark of
\citet{saha2023} (AUC\,=\,0.88).
...
Applying spatial CV to a comparable RF model on a single basin (Beas fold)
yielded AUC\,=\,XX, which is XX higher/lower than the \citet{saha2023} baseline
and broadly consistent with the expected range of 5--10\% reduction from
...
\paragraph{Synthetic placeholder data.}
The model results presented here are based on real conditioning factor rasters
but a partly SAR-augmented flood inventory.
Sections marked with ``XX'' throughout the results section will be populated
with actual values after the complete SAR inventory is downloaded from GEE
and real model training is complete.
The pipeline scripts and methodological framework are fully operational.
